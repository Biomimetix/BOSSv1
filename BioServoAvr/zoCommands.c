...
/*03*/	zoCommandSet[ZO_COMMAND_SET_PROFILE_ACCELERATION] =	zoCommandSetProfileAcceleration;
...
/*06*/	zoCommandSet[ZO_COMMAND_SET_CURRENT_LIMIT_DURATION] = zoCommandSetCurrentLimitDuration;
...
/*08*/	zoCommandSet[ZO_COMMAND_ABSOLUTE_POSITION_MOVE] = zoCommandAbsolutePositionMove;
		zoCommandsSetByteCount[ZO_COMMAND_ABSOLUTE_POSITION_MOVE] = 8;						
/*09*/	zoCommandSet[ZO_COMMAND_RELATIVE_POSITION_MOVE] = zoCommandRelativePositionMove;
		zoCommandsSetByteCount[ZO_COMMAND_RELATIVE_POSITION_MOVE] = 8;						
...
/*0B*/  zoCommandSet[ZO_COMMAND_PROFILED_ABSOLUTE_POSITION_MOVE] =
		...
		zoCommandsSetByteCount[ZO_COMMAND_PROFILED_ABSOLUTE_POSITION_MOVE] = 8;
/*0C*/	zoCommandSet[ZO_COMMAND_PROFILED_RELATIVE_POSITION_MOVE] =
		zoCommandProfiledRelativePositionMove;
		zoCommandsSetByteCount[ZO_COMMAND_PROFILED_RELATIVE_POSITION_MOVE] = 8;
...
/*0E*/	zoCommandSet[ZO_COMMAND_SET_ABSOLUTE_POSITION_SETPOINT] =
		...
		zoCommandsSetByteCount[ZO_COMMAND_SET_ABSOLUTE_POSITION_SETPOINT] = 8;
/*0F*/	zoCommandSet[ZO_COMMAND_SET_RELATIVE_POSITION_SETPOINT] =
		zoCommandSetRelativePositionSetpoint;
		zoCommandsSetByteCount[ZO_COMMAND_SET_RELATIVE_POSITION_SETPOINT] = 8;
...
/*11*/	zoCommandSet[ZO_COMMAND_SET_PROFILED_ABSOLUTE_POSITION_SETPOINT] =
		...
		zoCommandsSetByteCount[ZO_COMMAND_SET_PROFILED_ABSOLUTE_POSITION_SETPOINT] = 8;
/*12*/	zoCommandSet[ZO_COMMAND_SET_PROFILED_RELATIVE_POSITION_SETPOINT] =	
		zoCommandSetProfiledRelativePositionSetpoint;
		zoCommandsSetByteCount[ZO_COMMAND_SET_PROFILED_RELATIVE_POSITION_SETPOINT] = 8;
...
/*1D*/	zoCommandSet[ZO_COMMANDS_SET_ANTI_WINDUP] =	zoCommandSetAntiWindup;
		zoCommandsSetByteCount[ZO_COMMANDS_SET_ANTI_WINDUP] = 4;
/*1E*/	zoCommandSet[ZO_COMMANDS_SET_REVERSE_MODE] = zoCommandSetReverseMode;
		zoCommandsSetByteCount[ZO_COMMANDS_SET_REVERSE_MODE] = 1;
...
/*73*/	zoCommandGet[ZO_COMMAND_GET_WARNING-ZO_PROTOCOL_GET_COMMANDS_START]=
		zoCommandGetWarning;
/*74*/  zoCommandGet[ZO_COMMANDS_GET_ANTI_WINDUP-ZO_PROTOCOL_GET_COMMANDS_START] =
		zoCommandGetAntiWindup;
/*75*/  zoCommandGet[ZO_COMMANDS_GET_REVERSE_MODE-ZO_PROTOCOL_GET_COMMANDS_START] =
	zoCommandGetReverseMode;	
...
/*06*/	void zoCommandSetCurrentLimitDuration(ZO_PROTOCOL_PACKET *packet)
{
	...
	enterCritical();
	Sms.Settings.CurrentLimitDuration = currLimDurMiliSecs;
	exitCritical();
}
...
/*1D*/	void zoCommandSetAntiWindup(ZO_PROTOCOL_PACKET *packet)
{
		
		u32 antiwindup;

		antiwindup = strToS32(packet->data);
		eeprom_write_dword((u32*)ZO_EEPROM_ADDRESS_ANTI_WINDUP, antiwindup);
		
		enterCritical();
		Sms.Pid.AntiWindUp=antiwindup;
		exitCritical();
		
}
/*1E*/	void zoCommandSetReverseMode(ZO_PROTOCOL_PACKET *packet)
{
	u08 reverse;

	reverse = packet->data[0];
	
	eeprom_write_byte((u08*)ZO_EEPROM_ADDRESS_REVERSE_MODE, reverse & 0x0F);

	enterCritical();
	Sms.Settings.ReverseMode = reverse;
	exitCritical();
}
...
{
	packet->byteCount = 8;
	u16ToStr(zoAdcRead(ZO_ADC_CHANNEL_6),packet->data);
...
/*6F*/  void zoCommandGetPosition(ZO_PROTOCOL_PACKET *packet)
{
	...
	s64ToStr(zoEncoderGetPos(),packet->data);
}
...
/*72*/	void zoCommandGetCurrent(ZO_PROTOCOL_PACKET *packet)
{
	...
	packet->byteCount = 2;
	u16 Amps = 0;
	Amps=zoAdcRead(ZO_ADC_CHANNEL_7)* ZO_DEFAULT_MC33887;

	u16ToStr(Amps,packet->data);
...
/*74*/	void zoCommandGetAntiWindup(ZO_PROTOCOL_PACKET *packet)
{
	packet->byteCount = 4;

		enterCritical();
	u32ToStr(Sms.Pid.AntiWindUp,packet->data);
	exitCritical();
	
	
}
/*75*/	void zoCommandGetReverseMode(ZO_PROTOCOL_PACKET *packet)
{
	packet->byteCount = 1;
	packet->data[0] = Sms.Settings.ReverseMode;
}
...
/*C9*/	void zoCommandGlobalStart(ZO_PROTOCOL_PACKET *packet)
	{
		enterCritical();
		Sms.BufferdSetPoint.type = ZO_SMS_SETPOINT_NONE;
		zoEncoderReset();
...
/*CA*/	void zoCommandGlobalHalt(ZO_PROTOCOL_PACKET *packet)
	{
		enterCritical();
		Sms.Control.State = ZO_SMS_CONTROL_POSITION_CONTROL;
		Sms.Pid.SetPoint = zoEncoderGetPos();
...
/*CB*/	void zoCommandGlobalStop(ZO_PROTOCOL_PACKET *packet)
	{
		enterCritical();
		Sms.Control.State = ZO_SMS_CONTROL_NO_CONTROL;
		exitCritical();
