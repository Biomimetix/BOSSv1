...
void zoSmsControlLoop(void)
{
	...
	if(Sms.Control.State == ZO_SMS_CONTROL_NO_CONTROL)
		return;
			...
			//Motor OverCurrent Check 
		if (zoAdcRead(ZO_ADC_CHANNEL_7) * ZO_DEFAULT_MC33887 > Sms.Settings.CurrentLimit)
		
	{
		if (Sms.Profile.OverCurrentState==FALSE)
		{
			zoSystemTimerTimeOutInit(&timer);
			Sms.Profile.OverCurrentState=TRUE;
		}
		
		if (zoSystemTimerTimeOutExpired(&timer,Sms.Settings.CurrentLimitDuration))
		{
			ZO_PROTOCOL_PACKET *dummy = 0;
			zoCommandStop(dummy);
			zoErrorPut(&zoSmsError,ZO_SMS_ERROR_MOTOR_OVERCURRENT);
			Sms.Profile.OverCurrentState=FALSE;
			timer=0;
		}
	}
...
void zoSmsInit(void)
{
	...
	Sms.Pid.AntiWindUp=Sms.Settings.AntiWindup;
	...
	//zoAdcChannelEnable(ZO_ADC_CHANNEL_0);	// HBridge Fault input active low 
	//zoAdcChannelEnable(ZO_ADC_CHANNEL_1);	// Analog 2 input
	//zoAdcChannelEnable(ZO_ADC_CHANNEL_2);	// Analog 3 input
	//zoAdcChannelEnable(ZO_ADC_CHANNEL_3);   // Analog 4 input 
	//zoAdcChannelEnable(ZO_ADC_CHANNEL_6);	// Analog 1 input
	zoAdcChannelEnable(ZO_ADC_CHANNEL_7);   // HBridge Current Input 
...
void zoSmsPinsInit(void)
{
	...
	//PB2: DIO4	    : Digital IO 4 initialized as input ***SPI does use SS but it is not required for the AS5045 in BOSS-1
	//PB1: DIO3		: Digital IO 3 initialized as input
	//PB0: DIO2		: Digital IO 2 initialized as input
 	
	DDRB = 0x00;	//0000.0000  | SM 0x04 
	PORTB |= 0x38;	//0011.1000  | SM 0x3E


	//PORTC pin assignments_______________________________________________________________
	//PC7: Current		: Current reading of H Bridge
	//Note Analog 1 is on ADC6 and has no need to be set
	//PC6: NOT USED     : 
	//PC5: SCL			: I2C clock, configured as input, pull-up enabled
	//PC4: SDA			: I2C data, configured as input, pull-up enabled
	//PC3: AIN4-ADC3	: Analog input 4
	//PC2: AIN3-ADC2	: Analog input 3
	//PC1: AIN2-ADC1	: Analog input 2
	//PC0: FAULT		: FAULT from motor controller --not used instead use PD6 
		
	DDRC = 0x00;	//0000.0000 0x00 | SM 0x00 0000.0000
	PORTC |= 0x30;	//0011.0001 0x31 | SM 0x30 0011.0000
	...
	//PD6: FAULT\	: Active low fault input from motor driver (not used with MCP17511),pull-up enabled
	...
	DDRD = 0x3E;   //0011.1110 0x3E | SM 0x3C  0011.1100   SM 32 0x3E 0011.1110
	PORTD = 0xC1;  //1100.0001 0xC1 | SM 0xC0  1100.0000   SM 32 0xC1 1100.0001
}
...
void zoSmsInitSettingsFromEeprom(void)
{
	...
	Sms.Settings.AntiWindup= eeprom_read_dword((u32*)ZO_EEPROM_ADDRESS_ANTI_WINDUP);
	Sms.Settings.ReverseMode= eeprom_read_byte((u08*)ZO_EEPROM_ADDRESS_REVERSE_MODE);
...
void zoSmsRestoreDefaults(void)
{
	...
	eeprom_write_dword((u32*)ZO_EEPROM_ADDRESS_ANTI_WINDUP, ZO_DEFAULT_ANTIWINDUP);
	eeprom_write_byte((u08*)ZO_EEPROM_ADDRESS_REVERSE_MODE, ZO_DEFAULT_REVERSE_MODE);
}
